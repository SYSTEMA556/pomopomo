...
import time
import threading
from PIL import ImageGrab, ImageTk, ImageChops, Image
import winsound  # 追加
    ...
    def monitor_loop(self):
        while self.monitoring:
            try:
                ...
                rect = win32gui.GetWindowRect(self.selected_hwnd)
                left, top, right, bottom = rect
                x1, y1, x2, y2 = self.selected_region  # ウインドウ内の相対座標
                region = (left + x1, top + y1, left + x2, top + y2)
                current_image = ImageGrab.grab(bbox=region)
                ...
                if self.last_image is not None:
                    ...
                    if diff.getbbox() is not None:
                        ...
                        # 変化を検出したら音を鳴らす
                        winsound.Beep(1000, 500)  # 周波数: 1000Hz, 長さ: 500ms
                    else:
